...
// get next file id
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr = "select max(id) from tabletItemsFiles";
...
{
	$queryStr	= "select tabletId from tabletItems where id = $itemId";
	$result		= commonDoQuery($queryStr);
	$row		= commonQuery_fetchRow($result);
	$tabletId	= $row[0];
...
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr   = "select max(pos) from tabletItemsFiles where itemId = $itemId";
$result		= commonDoQuery ($queryStr);
$row		= commonQuery_fetchRow ($result);
$pos	 	= $row[0] + 1;
...
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr   = "insert into tabletItemsFiles (id, itemId, fileType, fileSize, pos, file, sourceFile)
...
$upload = ftp_put($connId, "tabletsFiles/$picFile",
...
if ($fileType == "pic")
{
	...
	if ($picDimsRow['picDimension1'] != 0)
	{
		...
		$upload = ftp_put($connId, "tabletsFiles/$resizedFileName",
...
{
	$debug = fopen("/tmp/tabletsFiles.log", "a");
	fwrite ($debug, "$msg\n");
	fclose ($debug);
}
