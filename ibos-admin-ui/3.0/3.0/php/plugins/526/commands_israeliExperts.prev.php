...
function getMemberDetails ($xmlRequest)
{
	...
	if ($id == "")
		trigger_error (iconv("utf-8", "windows-1255", "חסר קוד מומחה לביצוע הפעולה"));
...
function editMember ($xmlRequest, $editType)
{
	...
	if ($username != "")
	{
		...
		{
			trigger_error (iconv("utf-8", "windows-1255", "כבר קיים מומחה עם שם משתמש זהה"));
		}
	}
	...
	if ($id == "")
		trigger_error (iconv("utf-8", "windows-1255", "חסר קוד מנוי לביצוע הפעולה"));
...
function sendMemberPasswordEmail ($xmlRequest)
{
	...
	foreach ($ids as $id)
	{
		...
		commonSendHtmlEmailBySMTP("info1@israeli-expert.co.il", "info1", $from, "info@israeli-expert.co.il", $email, $fullname, $subject, $html, true);
	}
}
    ...
    $excelHd = "<?xml version='1.0' encoding='ISO-8859-8' ?>												
				<Workbook xmlns=\"urn:schemas-microsoft-com:office:spreadsheet\"
					...
					<Row ss:Height=\"18\">
						...
						<Cell ss:MergeAcross=\"12\" ss:StyleID=\"sSection\"><Data ss:Type=\"String\">לשימוש פנימי</Data></Cell>
