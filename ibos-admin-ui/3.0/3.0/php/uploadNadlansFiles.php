...
// get next file id
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr = "select max(id) from nadlanItemsFiles";
...
{
	$queryStr	= "select nadlanId from nadlansItems where id = $itemId";
	$result		= commonDoQuery($queryStr);
	$row		= commonQuery_fetchRow($result);
	$nadlanId	= $row[0];

	$query		= "select picDimension1, picDimension2
				   from nadlans where id = $nadlanId";
...
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr   = "select max(pos) from nadlanItemsFiles where itemId = $itemId";
$result		= commonDoQuery ($queryStr);
$row		= commonQuery_fetchRow ($result);
$pos	 	= $row[0] + 1;
...
// ----------------------------------------------------------------------------------------------------------------------------
$queryStr   = "insert into nadlanItemsFiles (id, itemId, fileType, fileSize, pos, file, sourceFile)
...
$upload = ftp_put($connId, "nadlansFiles/$picFile",
...
if ($fileType == "pic")
{
	...
	if ($picDimsRow['picDimension1'] != 0)
	{
		...
		if ($dimRow1['forceSize'] == "1")
		{
			picsToolsForceResize($filePath . $_FILES['Filedata']['name'], $suffix, $dimRow1['width'], $dimRow1['height'],
		...
		$upload = ftp_put($connId, "nadlansFiles/$resizedFileName",
	...
	if ($picDimsRow['picDimension2'] != 0)
	{
		...
		if ($dimRow2['forceSize'] == "1")
		{
			picsToolsForceResize($filePath . $_FILES['Filedata']['name'], $suffix, $dimRow2['width'], $dimRow2['height'],
...
{
	$debug = fopen("/tmp/nadlansFiles.log", "a");
	fwrite ($debug, "$msg\n");
	fclose ($debug);
}
