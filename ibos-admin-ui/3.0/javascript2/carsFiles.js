...
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createTitles ()
{
...
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createSpanFile ()
{
...
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createTable ()
{
	filesTableId  	= pageObj.addTable ();
	
	pageObj.setLoadFunction  (filesTableId, "carsFiles_loadData");
...
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createTableButtons ()
{
	btn1	= {type			: "",
			   textHEB		: "הוספת קבצים",
			   action		: "carsFiles_openForm('addFiles')",
			   inPopup	    : false}

	btn2	= {type			: "update",
			   action		: "carsFiles_openForm('updateFile')"}

	btn3	= {type			: "delete",
			   action		: "carsFiles_deleteFile()"}

   	btn4	= {type			: "show",
			   action		: "carsFiles_showFile()"}
...
/* carsFiles_loadData																		*/ 
/* ---------------------------------------------------------------------------------------- */
function carsFiles_loadData () 
{
	serverObj.cleanRequest ();
	serverObj.addTag (itemIdCol.xmlTag, pageObj.getSelectedValueOf(tableId,itemIdCol.xmlTag));

	serverObj.sendRequest("cars.getCarFiles", pageObj.getTablePaging(filesTableId), "carsFiles_loadData_continue"); 
}

function carsFiles_loadData_continue (i)
{
...
/* carsFiles_openForm																		*/
/* ---------------------------------------------------------------------------------------- */
function carsFiles_openForm (type)
{
	globalType = type;
	...
	else
	{
		...
		serverObj.sendRequest  ("cars.getFileDetails", undefined, "after_loadFileDetails");
	}
}
...
function carsFiles_openForm_continue ()
{
	var type = globalType;
	...
	if (type == "addFiles")
	{
		...
		multiUploadStart("http://www.i-bos.net/admin/3.0/php/uploadCarsFiles.php?itemId=" + id, filesFormId);
	}

	carsFiles_createFormButtons 	(type);
	handleDisplay 		   			(type);
}

/* ---------------------------------------------------------------------------------------- */
/* carsFiles_createFormButtons																*/
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createFormButtons (type)
{
	btnsGroups = new Array ();
	...
	if (type == "addFiles")
	{
		btn1.action = "carsFiles_loadData();" + btn1.action;
...
/* ---------------------------------------------------------------------------------------- */
/* carsFiles_submitForm																		*/
/* ---------------------------------------------------------------------------------------- */
function carsFiles_submitForm ()
{
	...
	serverObj.sendRequest("cars.updateFile", undefined, "carsFiles_submitForm_continue");
}

function carsFiles_submitForm_continue (i)
{
	var responseXml = asyncResponseXml.getResponseXml (i);
...
/* ---------------------------------------------------------------------------------------- */
/* carsFiles_deleteFile																		*/ 
/* ---------------------------------------------------------------------------------------- */
function carsFiles_deleteFile () 
{
	...
	confirm("מחיקת הקובץ", "delete file", "carsFiles_deleteFile_confirm");
}

function carsFiles_deleteFile_confirm ()
{
	serverObj.cleanTags ();
	serverObj.addTag	(fileIdCol.xmlTag, pageObj.getSelectedValueOf(filesTableId,fileIdCol.xmlTag));

	serverObj.sendRequest("cars.deleteFile", undefined, "carsFiles_deleteFile_continue");
}

function carsFiles_deleteFile_continue (i)
{
	var responseXml = asyncResponseXml.getResponseXml (i);
...
function carsFiles_showFile ()
{
	...
	carsFiles_createShowPage    	();

	carsFiles_createShowButtons 	();
...
/* carsFiles_createShowPage																	*/
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createShowPage ()
{
...
/* carsFiles_createShowButtons																*/
/* ---------------------------------------------------------------------------------------- */
function carsFiles_createShowButtons ()
{
	btn1	= {type			: "back",
			   action		: "handleDisplay('files')"}
