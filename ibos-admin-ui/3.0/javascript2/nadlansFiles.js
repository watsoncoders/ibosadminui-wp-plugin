...
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createTitles ()
{
...
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createSpanFile ()
{
...
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createTable ()
{
	filesTableId  	= pageObj.addTable ();
	
	pageObj.setLoadFunction  (filesTableId, "nadlansFiles_loadData");

	// handle the table
	// ------------------------------------------------------------------------------------
	column1		= {textHEB	: fileIdCol.textHEB,		widthHEB : 80,
				   textENG	: fileIdCol.textENG,		widthENG : 80,
	...
	column3		= {textHEB	: fileTypeCol.textHEB,		widthHEB : 100,
				   textENG	: fileTypeCol.textENG,		widthENG : 100,
				   sortType : "text",					sortDir  : "ascending",
				   xmlTag   : fileTypeCol.xmlTag}

	column4		= {textHEB	: fileSizeCol.textHEB,		widthHEB : 100,
				   textENG	: fileSizeCol.textENG,		widthENG : 100,
				   sortType : "text",					sortDir  : "ascending",
				   xmlTag   : fileSizeCol.xmlTag}

	column5		= {textHEB	: filePosCol.textHEB,		widthHEB : 100,
				   textENG	: filePosCol.textENG,		widthENG : 100,
				   sortType : "text",					sortDir  : "ascending",
...
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createTableButtons ()
{
	btn1	= {type			: "",
			   textHEB		: "הוספת קבצים",
			   action		: "nadlansFiles_openForm('addFiles')",
			   inPopup	    : false}

	btn2	= {type			: "update",
			   action		: "nadlansFiles_openForm('updateFile')"}

	btn3	= {type			: "delete",
			   action		: "nadlansFiles_deleteFile()"}

   	btn4	= {type			: "show",
			   action		: "nadlansFiles_showFile()"}
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_loadData																	*/ 
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_loadData () 
{
	serverObj.cleanRequest ();
	serverObj.addTag (itemIdCol.xmlTag, pageObj.getSelectedValueOf(tableId,itemIdCol.xmlTag));

	serverObj.sendRequest("nadlans.getNadlanFiles", pageObj.getTablePaging(filesTableId), "nadlansFiles_loadData_continue"); 
}

function nadlansFiles_loadData_continue (i)
{
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_openForm																	*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_openForm (type)
{
	globalType = type;
	...
	if (type == "addFiles")
	{
		nadlansFiles_openForm_continue ();
	}
	else	// update file
	{
		...
		serverObj.sendRequest  ("nadlans.getFileDetails", undefined, "after_loadFileDetails");
	}
}
...
function nadlansFiles_openForm_continue ()
{
	var type = globalType;
	...
	if (type == "addFiles")
	{
		...
		fieldsWidths = {HEB : new Array(80,500),
						ENG : new Array(80,500)}
	...
	pageObj.generateForm  (filesFormId,true);

	if (type == "addFiles")
	{
		var id = pageObj.getSelectedValueOf(tableId,itemIdCol.xmlTag);

		multiUploadStart("https://www.i-bos.co.il/3.0/php/uploadNadlansFiles.php?itemId=" + id, filesFormId);
	}

	nadlansFiles_createFormButtons 	(type);
	handleDisplay 		   			(type);
}

/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_createFormButtons															*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createFormButtons (type)
{
	btnsGroups = new Array ();
	...
	if (type == "addFiles")
	{
		btn1.action = "nadlansFiles_loadData();" + btn1.action;
	...
	{
		btn2 = {type		: "update",
			    action		: "nadlansFiles_submitForm()"}
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_submitForm																	*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_submitForm ()
{
	...
	serverObj.sendRequest("nadlans.updateFile", undefined, "nadlansFiles_submitForm_continue");
}

function nadlansFiles_submitForm_continue (i)
{
	var responseXml = asyncResponseXml.getResponseXml (i);
...
/* nadlansFiles_doRefresh																	*/ 
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_doRefresh() 
{
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_deleteFile																	*/ 
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_deleteFile () 
{
	...
	confirm("מחיקת הקובץ", "delete file", "nadlansFiles_deleteFile_confirm");
}

function nadlansFiles_deleteFile_confirm ()
{
	id 	= pageObj.getSelectedValueOf(filesTableId,fileIdCol.xmlTag);
	...
	serverObj.sendRequest("nadlans.deleteFile", undefined, "nadlansFiles_deleteFile_continue");
}

function nadlansFiles_deleteFile_continue (i)
{
	var responseXml = asyncResponseXml.getResponseXml (i);
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_showPage																	*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_showPage ()
{
...
/* ---------------------------------------------------------------------------------------- */
/* nadlansFiles_showFile																	*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_showFile ()
{
	if (!pageObj.isRowSelected(filesTableId)) 
	{
		commonMsgBox ("info", "יש לבחור קובץ");
	...
	nadlansFiles_createShowPage    	();

	nadlansFiles_createShowButtons 	();
...
/* nadlansFiles_createShowPage																*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createShowPage ()
{
...
/* nadlansFiles_createShowButtons															*/
/* ---------------------------------------------------------------------------------------- */
function nadlansFiles_createShowButtons ()
{
	btn1	= {type			: "back",
			   action		: "handleDisplay('files')"}
