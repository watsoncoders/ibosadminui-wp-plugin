...
{
    // Ensure inputs are treated as strings/integers for SQL safety
    $rewriteName = commonFixRewriteName((string)$rewriteName);
    $pageId      = (int)$pageId;

    if ($rewriteName !== "")
    {
        // Querying the pages_byLang table to ensure the rewrite name is unique
        $sql    = "SELECT * FROM pages_byLang WHERE rewriteName = '$rewriteName' AND pageId != $pageId AND pageId > 0";
        $result = commonDoQuery($sql);

        // Returning true if no other page uses this name
