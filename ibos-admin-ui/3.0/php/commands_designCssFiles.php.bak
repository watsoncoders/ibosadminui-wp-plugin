...
function getFiles ($xmlRequest)
{
	...
	$files = ftp_rawlist($connId, "-a css");
   	...
   	foreach ($files as $file) 
	{
       ...
       if ($split[0] != "total" && $split[0]{0} !== "d") 
	   {
			...
			{
		   		$date 		= date("d/m", strtotime($split[6] . $split[5])) . " " . $split[7];
				$fileName 	= $split[8];
				$fileSize 	= $split[4];
				$perms		= $split[0];
			}
			...
			$fullFileName = "";//commonGetDomainName($domainRow) . "/css/" . urlencode($split[8]);
			$fullFileName = commonGetDomainName($domainRow) . "/css/$fileName";
...
/* getFile																								*/
/* ----------------------------------------------------------------------------------------------------	*/
function getFile ($xmlRequest)
{
	$fileName   = commonDecode(xmlParser_getValue ($xmlRequest, "fileName"));
	...
	ftp_get ($connId, "/tmp/${fixedDomainName}.css", "css/$fileName", FTP_BINARY);

	$file		= fopen("/tmp/${fixedDomainName}.css", "r");
	...
	return "<content>"  . commonValidXml($content)  . "</content>
			<fileName>" . commonValidXml($fileName) . "</fileName>";
}
...
{
	$fileName   = commonDecode(xmlParser_getValue ($xmlRequest, "fileName"));
	$content    = commonDecode(xmlParser_getValue ($xmlRequest, "content"));
	...
	ftp_put ($connId,"css/$fileName","/tmp/${fixedDomainName}.css", FTP_BINARY);
